...
void MemoryAllocation::AllocateMemory(Json::Value &DNNInfo)
{
    ...
    GetInstructionGroupNum(DNNInfo);
    GetReloadInfo(DNNInfo);
//    AllocateNaive(DNNInfo);
    MemoryUsageInfo(DNNInfo);
}

void MemoryAllocation::GetInstructionGroupNum(Json::Value &DNNInfo)
{
...
int visited_record[MAX_AG] = {0};
float memory_usage_input[MAX_AG] = {0};
float memory_usage_output[MAX_AG] = {0};
float memory_usage_node[MAX_NODE] = {0};
float memory_usage_recv[MAX_CORE] = {0};
void MemoryAllocation::MemoryUsageInfo(Json::Value &DNNInfo)
{
        ...
        for (int j = 0; j < instruction_num; ++j)
        {
            ...
            if (strcmp(Instruction["operation"].asCString(), "MVMUL") == 0)
            {
                if (strcmp(Instruction["conv_or_fc"].asCString(), "OP_CONV") == 0)
                {
                    ...
                    if (visited_record[AG_index_in_total] == 0)
                    {
                        visited_record[AG_index_in_total] = 1;
                        ...
                        for (int k = 0; k < DNNInfo["6_recv_info"]["node_list"][node_index].size(); ++k)
                        {
                            ...
                            {
                                memory_usage_input[AG_index_in_total] += row_num * input_width * input_channel;
...
//                            }
//                        }
                        memory_usage_input[AG_index_in_total] += Instruction["input_element_num"].asInt();
                        memory_usage += Instruction["input_element_num"].asInt();
                        memory_usage_output[AG_index_in_total] += operation_cycle_before_comm * instruction_group_num * Instruction["output_element_num"].asInt();
                ...
                else
                {
                    int AG_index_in_total = Instruction["AG_index_in_total"].asInt();
                    memory_usage_input[AG_index_in_total] += Instruction["input_element_num"].asInt();
                    memory_usage_output[AG_index_in_total] += Instruction["output_element_num"].asInt();
                    memory_usage += Instruction["input_element_num"].asInt();
            ...
            else if (strcmp(Instruction["operation"].asCString(), "RECV") == 0)
            {
                ...
                if (strcmp(NodeList[node_index]["operation"].asCString(), "OP_CONV") == 0)
                {
                    ...
                    memory_usage_recv[i] += instruction_group_num * Instruction["element_num"].asInt();
                }
                else if (strcmp(NodeList[node_index]["operation"].asCString(), "OP_FC") == 0)
                {
                    memory_usage += Instruction["element_num"].asInt();
                    memory_usage_recv[i] += Instruction["element_num"].asInt();
                }
            }
        }
    ...
    {
        std::cout << "AG" << j
                  << "  input:" << std::left << std::setw(8) << memory_usage_input[j] * 16 / 8 / 1024 << "KB"
                  << "  output:" << std::left  << std::setw(8) << memory_usage_output[j] * 16 / 8 / 1024 << "KB" << std::endl;
        ag_memory_sum += memory_usage_input[j] + memory_usage_output[j];
    }
    ...
    for (int i = 0; i < effective_node_num; ++i)
    {
        ...
        for (int j = 0; j < replication_num; ++j)
        {
            ...
            for (int k = 0; k < AG_num_in_replication; ++k)
            {
                ...
                memory_usage_node[node_index] += memory_usage_input[AG_index_in_total] + memory_usage_output[AG_index_in_total];
            }
        }
        node_memory_sum += memory_usage_node[node_index];
        std::cout << "node" << node_index << "  "  << memory_usage_node[node_index] * 16 / 8 / 1024 << "KB" << std::endl;
    }
    ...
    for (int i = 0; i < core_num; ++i)
    {
        std::cout << " core" << i <<  "  " << memory_usage_recv[i] * 16 / 8 / 1024 << "KB" << std::endl;
        core_recv_sum += memory_usage_recv[i];
    }
    std::cout << "sum:" << core_recv_sum * 16 / 8 / 1024 << "KB" << std::endl;
}

static int core_AG_num[MAX_CORE] = {0};
void MemoryAllocation::GetReloadInfo(Json::Value &DNNInfo)
{
    int effective_node_num = DNNInfo["2_effective_node"].size();
    for (int i = 0; i < effective_node_num; ++i)
    {
        ...
        if (strcmp(NodeList[effective_node_index]["operation"].asCString(), "OP_CONV") == 0)
        {
            ...
            for (int j = 0; j < reload_AG_num; ++j)
            {
                ...
                int reload_num = ceil(DNNInfo["2_AG_partition"][i]["replication"][replication_index]["instruction_group_num"].asFloat() / appointed_instruction_group_num);
                Json::Value Reload;
...
void MemoryAllocation::AllocateNaive(Json::Value &DNNInfo)
{
    int instruction_group_num = BackBone.size();
    DNNInfo["8_instruction_ir_with_reload"].resize(instruction_group_num);
            ...
            for (int j = 0; j < core_num; ++j)
            {
                ...
                for (int k = 0; k < reload_AG_num; ++k)
                {
                    ...
                    if (strcmp(NodeList[node_index]["operation"].asCString(), "OP_CONV") == 0)
                    {
                        ...
                        DNNInfo["8_instruction_ir_with_reload"][i]["core_list"][j].append(Instruction_ld);
                    }
                    else if(strcmp(NodeList[node_index]["operation"].asCString(), "OP_FC") == 0)
                    {
                        if (reload_index >= 1)
                ...
                for (int k = 0; k < instruction_ir_num; ++k)
                {
                    ...
                    DNNInfo["8_instruction_ir_with_reload"][i]["core_list"][j].append(tmpInstruction);
                }
            }
        }
            ...
            for (int j = 0; j < core_num; ++j)
            {
                ...
                for (int k = 0; k < reload_AG_num; ++k)
                {
                    ...
                    if (strcmp(NodeList[node_index]["operation"].asCString(), "OP_CONV") == 0)
                    {
                        ...
                        DNNInfo["8_instruction_ir_with_reload"][i]["core_list"][j].append(Instruction_st);
                    }
                    else if(strcmp(NodeList[node_index]["operation"].asCString(), "OP_FC") == 0)
                    {
                        if (reload_index >= 1)
...
void MemoryAllocation::ShowInstruction(Json::Value &DNNInfo)
{
    for (int inf = inference_start; inf <= inference_end ; ++inf)
    {
        ...
        int instruction_group_num = static_cast<int>(DNNInfo["8_instruction_ir_with_reload"].size());
        for (int i = 0; i < instruction_group_num; ++i)
        {
            std::cout << std::endl;
            ...
            for (int j = 0; j < core_num; ++j)
            {
                ...
                int instruction_num = DNNInfo["8_instruction_ir_with_reload"][i]["core_list"][j].size();
                for (int k = 0; k < instruction_num; ++k)
                {
                    Json::Value Instruction = DNNInfo["8_instruction_ir_with_reload"][i]["core_list"][j][k];
