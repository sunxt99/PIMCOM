...
{
    if (FastMode)
        PartitionNaiveFast(DNNInfo);
    else
        PartitionNaiveSlow(DNNInfo);
    Check();
    ArrayGroupIndex = 0;
    VirtualCrossbarIndex = 0;
}

void CrossbarPartition::PartitionNaiveFast(Json::Value &DNNInfo)
{
...
void CrossbarPartition::PartitionNaiveSlow(Json::Value &DNNInfo)
{
